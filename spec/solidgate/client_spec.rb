...
RSpec.describe Solidgate::Client do
  ...
  describe "#decrypt_intent" do
    let(:intent_params) do
      { order_id: "order_123", product_id: "prod_456" }.to_json
    end

    it "decrypts a payload generated by generate_intent" do
      encrypted = client.generate_intent(intent_params)
      decrypted = client.decrypt_intent(encrypted)

      expect(decrypted).to eq(intent_params)
    end

    it "raises argument error for invalid payload" do
      expect { client.decrypt_intent("not_base64") }
        .to raise_error(ArgumentError, "Invalid encrypted payload")
    end
  end
